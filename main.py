...
class SearchAndDestroy():
    def __init__(self, dimensions, rule, target_type=None):
        self.dim = dimensions
        self.target = ()
...
if __name__ == "__main__":
    ...
    if args.question == "basic":
        game = SearchAndDestroy(dimensions=int(args.grid_dimension), rule=args.rule, target_type=None)
        agent = Agent(game)
            ...
            for rule in ["belief", "confidence"]:
                for terrain_target in ["flat", "hill", "forest", "cave"]:
                    ...
                    for iter in range(10):
                        game = SearchAndDestroy(dimensions=grid_size, rule=rule, target_type=terrain_target)
                        agent = Agent(game)
                        agent_iters += agent.run_game(rule_type="normal")
